...
function AccountSecuritySection() {
  ...
  const validate = () => {
    ...
    if (!formValues.newPassword.trim()) {
      ...
      const pw = formValues.newPassword;

      if (pw.length < 8) {
        newErrors.newPassword = 'New password must be at least 8 characters.';
      } else if (!/[A-Z]/.test(pw)) {
        newErrors.newPassword = 'New password must include at least 1 uppercase letter.';
      } else if (!/[a-z]/.test(pw)) {
        newErrors.newPassword = 'New password must include at least 1 lowercase letter.';
      } else if (!/[0-9]/.test(pw)) {
        newErrors.newPassword = 'New password must include at least 1 number.';
      } else if (!/[!@#$%^&*(),.?":{}|<>_\-\\[\]\/~`+=;]/.test(pw)) {
        newErrors.newPassword = 'New password must include at least 1 special character.';
      } else if (formValues.currentPassword && pw === formValues.currentPassword) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ success message will render BELOW Change Password section
      setMessage({
        type: 'success',
  ...
  return (
    <div className="w-full mx-auto bg-white border shadow-sm border-slate-200 lg:rounded-r-xl rounded-b-xl">
      ...
      <div className="px-4 py-4 sm:px-6">
        ...
        <p className="mt-1 text-xs text-tertiary sm:text-sm">Manage your password, MFA, and session activity.</p>
      </div>
      ...
      <div className="px-4 py-4 space-y-5 sm:px-6 sm:py-6">
        ...
        <section className="border rounded-lg border-slate-200">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 sm:px-5">
            ...
            <h3 className="text-sm font-semibold text-secondary sm:text-base">Change Password</h3>
          </div>

          <form onSubmit={handleSubmit} className="px-4 py-4 space-y-4 sm:px-5 sm:py-5">
            {/* Current Password */}
            <div className="space-y-1">
              <label htmlFor="currentPassword" className="text-xs font-medium text-slate-700 sm:text-sm">
                Current Password
              </label>
            ...
            {/* New Password */}
            <div className="space-y-1">
              <label htmlFor="newPassword" className="text-xs font-medium text-slate-700 sm:text-sm">
                New Password
              </label>
              <div className="relative">
                <input
            ...
            <div className="pt-2 space-y-2">
              ...
              <p className="text-xs text-tertiary sm:text-sm">Last updated: {lastUpdated}</p>
            </div>
          </form>

          {/* ✅ message placed BELOW Change Password section */}
          {message && (
            <div
              className={`mx-4 mb-4 sm:mx-5 rounded-md px-3 py-2 text-xs sm:text-sm ${
          ...
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              <div className="flex items-start gap-2">
                <img src="shield-zap.svg" alt="MFA" className="w-5 h-5 mt-1" />
                <div>
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full border transition-colors ${
                isMfaEnabled ? 'bg-primary border-primary' : 'bg-slate-200 border-slate-300'
              }`}
            >
              <span
        ...
        <section className="w-full px-6 py-6 bg-white border shadow-sm rounded-xl border-slate-200">
          {/* Title row */}
          <div className="flex items-center gap-3">
            <img src="/session-icon.svg" alt="Session control" className="w-5 h-5 shrink-0" />
            <h3 className="text-sm font-semibold text-secondary sm:text-base">Session Control</h3>
          </div>

          {/* Description */}
          <p className="mt-2 text-xs leading-relaxed sm:text-sm text-tertiary">
            Logout from all devices where you are currently signed in. You will need to log in again on this device.
          </p>
          ...
          <div className="mt-4">
            <button
              ...
              disabled={loggingOut}
              className="inline-flex items-center justify-center rounded-lg bg-primary px-6 py-3 text-[14px] font-semibold text-white hover:bg-[#0B2547] disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        {/* Global message (kept as-is, but will only show if you set message elsewhere) */}
        {/* NOTE: Password message already shows below Change Password section as requested */}
      </div>
    </div>
  );
}
